...
def process_extra_span(string_instance):
    ...
    if string_instance['question'] == "what state's capital is dover ?":
        span = (1, 3)
        idx = 0
        while idx < len(string_instance['induced_spans']):
            if int(string_instance['induced_spans'][idx]['span'][0]) == span[0] and int(string_instance['induced_spans'][idx]['span'][1]) == span[1]:
                break
            idx += 1
        string_instance['induced_spans'].pop(idx)
        start = span[0]
        diff = span[1] - span[0]
    return start, diff 
...
with open(args.train) as reader:
    for line in reader:
        ...
        '''for item in string_instance['induced_spans']:
            item['span'][1] = int(item['span'][1])-1
        if string_instance['question'] == "in which state does the highest point in usa exist ?":
            string_instance['induced_spans'] = json.loads('[{"span": [0, 10], "type": "span"}, {"span": [0, 0], "type": "loc_1"}, {"span": [2, 2], "type": "state"}, {"span": [0, 1], "type": "span"}, {"span": [2, 4], "type": "span"}, {"span": [5, 10], "type": "span"}, {"span": [5, 5], "type": "highest"}, {"span": [6, 8], "type": "span"}, {"span": [6, 6], "type": "place"}, {"span": [7, 8], "type": "span"}, {"span": [7, 7], "type": "loc_2"}, {"span": [8, 8], "type": "countryid#\'usa\'"}]')
        if string_instance['question'] == "what are the population densities of each us state ?":
            string_instance['induced_spans'] = json.loads('[{"span": [0, 9], "type": "span"}, {"span": [2, 9], "type": "span"}, {"span": [2, 4], "type": "density_1"}, {"span": [8, 8], "type": "state"}, {"span": [2, 7], "type": "span"}, {"span": [8, 9], "type": "span"}]')
        if string_instance['question'] == "what river traverses the state which borders the most states ?":
            string_instance['induced_spans'] = json.loads('[{"span": [0, 10], "type": "span"}, {"span": [1, 10], "type": "span"}, {"span": [1, 1], "type": "river"}, {"span": [2, 10], "type": "span"}, {"span": [2, 2], "type": "traverse_2"}, {"span": [2, 3], "type": "span"}, {"span": [4, 10], "type": "span"}, {"span": [8, 8], "type": "most"}, {"span": [9, 9], "type": "state"}, {"span": [4, 6], "type": "span"}, {"span": [9, 10], "type": "span"}, {"span": [4, 4], "type": "state"}, {"span": [6, 6], "type": "next_to_2"}, {"span": [5, 6], "type": "span"}, {"span": [7, 8], "type": "span"}]')'''
        print(string_instance['question'])
        print(string_instance['program'])
        print(string_instance['induced_spans'])
        print()
        tree = Tree()
        '''if int(string_instance['induced_spans'][0]['span'][0])==0 and int(string_instance['induced_spans'][0]['span'][1])==len(string_instance['question'].split())-1:
            flag = 0
        else:
            flag = 1
        diff = 0
        ...
        '''for i, t in enumerate(string_instance['question'].split()):
            ...
            while True:
                ...
                #tree.show()
                break'''       
        all_sem.append((string_instance['question'], string_instance['program'], string_instance['induced_spans'], tree))     
        #tree.show()

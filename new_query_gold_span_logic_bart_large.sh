TASK=data/geo_fun_template_gold_span
PARAM='-large-share-ebd'
...
do 
    echo '------------------------------------------------------------'
    echo $f
    CUDA_VISIBLE_DEVICES=$1 python /home/ec2-user/quic-efs/user/jingfe/sem_parses/my_code/semantic_parsing.py --model-dir $(dirname $f) --model-file $(basename $f) --src $TASK/src.test --out $TASK/pred$PARAM.test --data-dir "${TASK}/bin-large/" 
    python acc_deno.py -pred $TASK/pred$PARAM.test -gold $TASK/tgt.test
done
